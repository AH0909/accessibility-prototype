...
it('renders solution text', () => {
  ...
  expect(solutionText.text()).toEqual(`Prioritised solutionsAfter analysing the results by taking the 'range of ratings' and the 'average rating' into consideration, I prioritised the accessibility features. In the time period between the questionnaire and focus group, I implemented as many solutions as possible, starting with the ability to navigate with a keyboard '1'. The focus group was then able to give me feedback in order to further refine the accessibility features implementedThis prototype will then be used to create a report feeding back the answer to my research question.`)
});
...
it('renders video text', () => {
  ...
  expect(videoText.text()).toEqual(`W3C, known as the World Wide Web Consortium, is an international standards organisation for the Web.3C have produced a set of standards for accessibility named Web Content Accessibility Guidelines (WCAG).The most recent set of guidelines are WCAG 2.1 published in June 2018.   As part of my project I have looked closely at navigation and colour.For navigation, \"Success Criterion 3.2.3 Consistent Navigation\" is relevant. For colour, \"Understanding Success Criterion 1.4.3: Contrast (Minimum)\" is relevant.`)
});
